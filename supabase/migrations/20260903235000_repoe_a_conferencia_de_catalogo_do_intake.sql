-- ============================================================================
-- REPARO DE COLISÃO ENTRE SESSÕES.
--
-- Duas sessões mexeram na `fn_intake_proposta` no mesmo dia, e a minha apagou a
-- da outra em silêncio.
--
-- Ordem do que aconteceu:
--   1. Eu apliquei 20260902102000 + 20260903093000 (fila do OEM, valor por
--      módulo, guarda de contradição, data da venda).
--   2. A outra sessão leu a produção com essas mudanças e aplicou
--      20260903123000, que é um SUPERCONJUNTO da minha: mantém tudo e acrescenta
--      a conferência de catálogo por tenant (`v_chks`), a conferência de formato
--      de data e número, o aviso `cadastro_incompleto` e a mensagem de erro do
--      up-sell dizendo o que o cliente tem.
--   3. Eu apliquei 20260903100000 (tirar `origem='intake'` do payload da fila)
--      montada sobre o dump do PASSO 1. `CREATE OR REPLACE` não mescla: o corpo
--      inteiro foi substituído e as ~200 linhas do passo 2 sumiram, sem erro
--      nenhum.
--
-- Esta migration repõe o corpo do passo 2 COM a correção do passo 3 aplicada
-- por cima — é o único ponto em que as duas coexistem. Confirmado antes de
-- escrever: a versão da outra sessão não removeu nada do que eu tinha feito, e
-- já traz o `coalesce(v_mva, 0)` da correção da constraint de ativação.
--
-- A lição, que já estava anotada e eu não segui: **releia a produção
-- imediatamente antes de aplicar um CREATE OR REPLACE**, não no começo do
-- trabalho. O dump que eu usei tinha 12 horas.
-- ============================================================================
