...
begin
  ...
  v_motivo := case
    ...
    when coalesce(v_recorr, 'mensal') <> 'mensal' then
      format('Contrato com recorrência "%s" — o portal informa valor mensal.', v_recorr)
  end;
  ...
  if 'custo' = any(p_acoes) then
    if v_motivo is not null then
      ...
      update public.cliente_produtos set vlr_custo = r.custo_hiper, updated_at = now() where id = v_cp_id;
  ...
  if 'mrr' = any(p_acoes) then
    if v_motivo is not null then
      ...
      update public.cliente_produtos set vlr_mensal = r.mrr_hiper, updated_at = now() where id = v_cp_id;
