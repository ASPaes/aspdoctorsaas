-- DEM-0315: filtro de categoria e subcategoria na aba Tickets do Dashboard de Atendimento.
--
-- Corpo copiado da PRODUÇÃO em 18/09/2026 (pg_get_functiondef), não do repo.
-- Única mudança: 2 parâmetros no fim (p_category_ids, p_subcategory_ids) e 2
-- linhas no WHERE do CTE tk. Categoria e subcategoria combinam com AND.
--
-- DROP antes do CREATE: adicionar parâmetro com CREATE OR REPLACE criaria uma
-- 2ª sobrecarga, e a chamada nomeada do PostgREST ficaria ambígua.
-- Grants iguais aos de hoje: authenticated + service_role, sem anon/PUBLIC.

BEGIN;

DROP FUNCTION public.get_atendimento_taxonomia(uuid, timestamptz, timestamptz, uuid, bigint, uuid, bigint[], bigint[], bigint[], bigint[], bigint[], bigint[], text);
...
REVOKE ALL ON FUNCTION public.get_atendimento_taxonomia(uuid, timestamptz, timestamptz, uuid, bigint, uuid, bigint[], bigint[], bigint[], bigint[], bigint[], bigint[], text, uuid[], uuid[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_atendimento_taxonomia(uuid, timestamptz, timestamptz, uuid, bigint, uuid, bigint[], bigint[], bigint[], bigint[], bigint[], bigint[], text, uuid[], uuid[]) TO authenticated, service_role;
