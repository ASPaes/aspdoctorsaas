-- 20/08/2026 -- montar_payload_contrato_omie: recusar contrato com valor mensal zerado.
--
-- Motivo: o payload leva valor_mensal = calcular_mrr_cliente(cliente). Quando o produto do
-- cliente esta sem vlr_mensal, saia valorUnit/valorTotal/nValTotMes = 0 e o Omie devolvia
-- "ERROR: E obrigatorio informar o valor unitario do item, tag [valorUnit], na inclusao!".
-- Nenhuma validacao daqui pegava isso. Passa a virar erro legivel, como os demais.
--
-- CREATE OR REPLACE (sem DROP) preserva os GRANTs de authenticated/service_role.
-- Cancelamento nao e afetado: contrato cancelado retorna antes deste bloco.
...
BEGIN
  ...
  -- 20/08/2026: o calculo do MRR subiu para ca (era feito depois do bloco de erros) para poder
  -- recusar contrato com valor zero ANTES de montar o payload. Sem isto o contrato ia inteiro
  -- para o Omie e voltava com "ERROR: E obrigatorio informar o valor unitario do item, tag
  -- [valorUnit], na inclusao!" -- faultstring crua, que a tela nem mostrava (contrato 2026-5700,
  -- LA ESTANCIA, DigiUp). Em contrato que JA existe no Omie o estrago seria pior: gravar zero
  -- zera a cobranca. Aqui a recusa sai em portugues, junto com os outros motivos.
  v_valor_mrr := public.calcular_mrr_cliente(v_cliente.id, p_tenant_id);
  IF COALESCE(v_valor_mrr, 0) <= 0 THEN
    v_erros := array_append(v_erros,
      'Valor mensal do cliente esta em R$ ' ||
      replace(trim(to_char(COALESCE(v_valor_mrr, 0), 'FM999999990.00')), '.', ',') ||
      '. O Omie recusa item de contrato sem valor unitario, e num contrato ja existente gravar ' ||
      'zero pararia a cobranca. Preencha o valor mensal do produto do cliente antes de sincronizar.');
  END IF;
