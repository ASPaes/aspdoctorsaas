-- DEM-0374 — o intake passa a copiar a quantidade do modulo para a jornada.
--
-- Unica mudanca no corpo (bloco "Modulos da jornada"): o INSERT em
-- onboarding_journey_modules agora leva `quantidade`, vinda da propria linha de
-- cliente_produto_modulos que ele acabou de gravar a partir do payload da
-- calculadora. O greatest/coalesce e cinto de seguranca — a coluna e NOT NULL
-- na ficha, mas o CHECK da jornada exige >= 1.
--
-- Corpo extraido da PRODUCAO (supabase db dump --linked, 08/09/2026,
-- md5(prosrc) = 4b8c8bdb7da114e64e7b8094ce9e3b04) e nao do repositorio, para
-- nao desfazer alteracao aplicada direto no banco.
--
-- Depende da coluna criada em 20260908210000 — aplicar naquela ordem.
