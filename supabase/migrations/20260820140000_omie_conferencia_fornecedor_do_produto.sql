-- Conferencia Omie: o "Sem fornecedor" do painel lia um campo que ninguem escreve mais.
--
-- O QUE ACONTECIA. A deteccao pegava o fornecedor em `clientes.fornecedor_id` -- coluna LEGADA.
-- O cadastro grava em `cliente_produtos.fornecedor_id` (ClienteProdutosSection, "Editar Produto"),
-- e o formulario do cliente tem whitelist explicita que EXCLUI fornecedor_id do UPDATE
-- (ClienteForm.tsx: "campos legados migrados para cliente_produtos"). Ou seja: nenhuma tela do
-- sistema alimenta a coluna que o painel lia. Todo cliente cadastrado depois dessa migracao cai
-- na pill "Sem fornecedor" mesmo com o fornecedor preenchido na tela.
--
-- Caso real (20/08/2026): RESTAURANTE CASA CAJU LTDA (67.742.532/0001-75) aparece "sem fornecedor"
-- no card de divergencia; no banco, clientes.fornecedor_id = NULL e cliente_produtos.fornecedor_id
-- = 13 (PDV Legal), produto ativo de R$ 305,64. Nao e caso isolado: no tenant DigiOffice, 133
-- clientes estao com a coluna legada nula e 123 deles tem fornecedor no produto ativo.
--
-- O QUE MUDA. O fornecedor passa a vir do caminho que o cadastro realmente escreve, em 3 degraus:
--   1) produto vendido POR ESTE contrato (contrato_itens -> cliente_produtos). E o mais preciso:
--      cliente com 2 produtos de fornecedores diferentes tem 1 contrato para cada, e cada linha
--      do painel recebe o seu.
--   2) item sem vinculo (contrato_itens.cliente_produto_id NULL): cai para o produto ATIVO do
--      cliente, o de maior mensalidade.
--   3) legado `clientes.fornecedor_id` -- so quando os dois acima nao acham nada. Fica como
--      fallback porque a base antiga (a maioria dos clientes da Digi) ainda so tem ele.
--
-- Nada aqui muda dado de cliente, contrato ou vinculo Omie: e so classificacao/rotulo. Rodar a
-- deteccao da conta recalcula as pills.
--
-- Corpo copiado da funcao QUE ESTA EM PRODUCAO (dump de 20/08/2026). A unica alteracao esta na
-- CTE `m`: os dois LATERAL novos, o COALESCE do fornecedor e o join de `fornecedores` sobre ele.
...
-- Grants: repostos porque CREATE OR REPLACE nao os perde, mas um DROP futuro perderia. Os mesmos
-- que a funcao ja tem em producao (authenticated + service_role).
