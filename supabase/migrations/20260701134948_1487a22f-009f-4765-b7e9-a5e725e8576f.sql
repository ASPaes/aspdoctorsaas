...
BEGIN
  ...
  v_ticket_dept := (v_old_json->>'department_id')::uuid;

  SELECT f.department_id INTO v_user_dept
  FROM profiles p
  JOIN funcionarios f ON f.id = p.funcionario_id AND f.tenant_id = p.tenant_id
  WHERE p.user_id = v_uid AND p.tenant_id = v_tenant_id;
  ...
  IF NOT public.is_admin_or_head() AND NOT public.is_super_admin() THEN
    IF v_resp_uid IS DISTINCT FROM v_uid
       AND NOT v_is_mentioned
       AND (v_user_dept IS NULL OR v_ticket_dept IS DISTINCT FROM v_user_dept) THEN
      RAISE EXCEPTION 'Sem permissão para alterar este ticket';
    END IF;
  END IF;
  ...
  LOOP
    IF v_key = 'status_id' OR NOT (v_key = ANY(v_allowed_fields)) THEN
      CONTINUE;
    END IF;
    ...
    ELSIF v_key IN ('data_inicio_implantacao','data_fim_implantacao') THEN
      EXECUTE format('UPDATE support_tickets SET %I = $1::date, atualizado_em = NOW() WHERE id = $2 AND tenant_id = $3', v_key)
        USING v_val, p_ticket_id, v_tenant_id;
    ELSIF v_key = 'prioridade' THEN
      EXECUTE 'UPDATE support_tickets SET prioridade = $1::support_ticket_prioridade, atualizado_em = NOW() WHERE id = $2 AND tenant_id = $3'
        USING v_val, p_ticket_id, v_tenant_id;
    ELSE
    ...
    INSERT INTO support_ticket_events (tenant_id, ticket_id, user_id, event_type, content, old_value, new_value)
    VALUES (v_tenant_id, p_ticket_id, v_uid, 'field_change', v_key, v_old_val, v_val);
  END LOOP;

  IF p_fields ? 'status_id' THEN
    v_key := 'status_id';
    v_val := p_fields->>'status_id';
    v_old_val := v_old_json->>v_key;

    IF v_old_val IS DISTINCT FROM v_val THEN
      IF v_val IS NULL OR v_val = '' THEN
        UPDATE support_tickets
        SET status_id = NULL, atualizado_em = NOW()
      ...
      ELSE
        ...
        IF COALESCE(v_is_terminal, false) THEN
          -- Combina a mudança de status e concluido_em em um único UPDATE
          -- para evitar que o trigger de proteção de tickets terminais
          -- bloqueie o segundo UPDATE (quando OLD.status_id já seria terminal).
          UPDATE support_tickets
        ...
        ELSE
          UPDATE support_tickets
          SET status_id = v_val::uuid, atualizado_em = NOW()
          WHERE id = p_ticket_id AND tenant_id = v_tenant_id;
        END IF;
      END IF;

      INSERT INTO support_ticket_events (tenant_id, ticket_id, user_id, event_type, content, old_value, new_value)
      VALUES (v_tenant_id, p_ticket_id, v_uid, 'status_change', v_key, v_old_val, v_val);
    END IF;
  END IF;
END;
$function$;