-- Venda nova da calculadora: custo da tabela do OEM e data da venda do dia.
--
-- 1) CUSTO. Os módulos nasciam com custo 0 e origem 'intake'. A carga do
--    espelho só mantém linha 'oem', então nem depois do vínculo da licença o
--    custo entrava: 12 clientes zerados em 17/09/2026 (COMIDARIA NA TORA entre
--    eles, R$ 0,00 na ficha contra R$ 99,90 no OEM). Agora, quando o produto
--    tem de-para com o OEM, a linha nasce 'oem' com o custo unitário da tabela
--    de preços da conta. A ficha soma pelo fn_sync_produto_valores; se o que a
--    licença cobra for diferente, a divergência aparece na conferência.
--
--    Produto do DS ligado a mais de um produto do OEM (PDV Legal e Raspberry
--    têm GESTAO LEGAL e FULL): vale o que tem mais licenças ativas na conta,
--    desempate pelo código (decisão do Alexandre, 17/09/2026: GESTAO LEGAL).
--
-- 2) DATA DA VENDA. Passa a ser o dia em que o pedido chega (= o ticket foi
--    finalizado na calculadora), no fuso de São Paulo. A data do payload é
--    ignorada só na venda nova; a alteração continua como estava.
...
DECLARE
  ...
  v_ticket uuid;
  v_f_fornec bigint; v_f_modelo bigint; v_f_recor text;
  -- custo da tabela do OEM na venda nova
  v_oem_conta uuid; v_oem_prod text; v_preco numeric; v_sem_preco text[];
BEGIN
    ...
    FOR v_prod IN SELECT jsonb_array_elements(p_payload->'produtos') LOOP
      ...
      v_cp := public.create_cliente_produto_with_contract(
        ...
        jsonb_build_object(
          ...
          -- Sem fornecedor no payload, cai para o do PRODUTO: desde 04/09/2026 a
          -- Calculadora nao manda mais esse campo, e os 6 produtos ja tem o seu
          -- no cadastro. A regra fica AQUI e nao na RPC canonica de proposito: a
          -- tela passa por la e o "-- Nenhum --" escolhido por uma pessoa tem
          -- que continuar valendo.
          'fornecedor_id',                  coalesce(
                                              nullif(coalesce(v_prod->>'fornecedor_id', v_com->>'fornecedor_id'),'')::bigint,
                                              (SELECT pr.fornecedor_id FROM produtos pr
                                                WHERE pr.id = (v_prod->>'produto_id')::bigint)),
          ...
          -- O dia em que o ticket foi finalizado na calculadora, que é o dia em
          -- que o pedido chega. A data do payload vinha de outro momento (a
          -- COMIDARIA chegou em 04/09 dizendo 02/09) e jogava a venda no mês
          -- errado.
          'data_venda',                     (now() AT TIME ZONE 'America/Sao_Paulo')::date,
      ...
      -- Conta do OEM pela unidade do cliente (mesma regra da fila) e, dentro
      -- dela, o produto do OEM deste produto. Sem conta ou sem de-para o
      -- produto não é licenciado: linha 'intake' sem custo, como antes.
      v_oem_conta := NULL; v_oem_prod := NULL; v_sem_preco := '{}';
      SELECT i.id INTO v_oem_conta
        FROM oem_integration i
        JOIN clientes cl ON cl.id = v_cliente
       WHERE i.tenant_id = v_tenant AND i.ativo = true
         AND i.unidades_base_ids @> ARRAY[cl.unidade_base_id]
       ORDER BY i.criado_em
       LIMIT 1;
      IF v_oem_conta IS NOT NULL THEN
        SELECT v.produto_codigo INTO v_oem_prod
         ...
         WHERE v.conta_integration_id = v_oem_conta
           AND v.produto_id = (v_prod->>'produto_id')::bigint
      ...
      FOR v_mod IN SELECT jsonb_array_elements(v_prod->'modulos') LOOP
        v_preco := NULL;
        IF v_oem_prod IS NOT NULL THEN
          SELECT pr.valor_unitario INTO v_preco
            FROM produto_modulos pm
            JOIN oem_espelho_modulo_preco pr
              ON pr.conta_integration_id = v_oem_conta
             AND pr.produto_codigo = v_oem_prod
             AND pr.modulo_codigo = pm.oem_modulo_codigo
           WHERE pm.id = (v_mod->>'modulo_id')::uuid
           LIMIT 1;
          IF v_preco IS NULL THEN
            v_sem_preco := array_append(v_sem_preco,
              coalesce((SELECT pm.nome FROM produto_modulos pm WHERE pm.id = (v_mod->>'modulo_id')::uuid),
                       v_mod->>'modulo_id'));
          END IF;
        END IF;
        -- 'oem' é quem MANTÉM a linha: depois do vínculo a carga do espelho
        -- troca este custo pelo que a licença cobra. A procedência da venda
        -- fica em cliente_produto_modulo_eventos.fonte ('calculadora').
        -- vlr_custo_total fica NULL: a ficha multiplica pela quantidade até o
        -- OEM mandar o total real.
        INSERT INTO cliente_produto_modulos (tenant_id, cliente_produto_id, modulo_id, quantidade, vlr_mensal, vlr_ativacao, vlr_custo, ativo, origem)
        VALUES (v_tenant, v_cp, (v_mod->>'modulo_id')::uuid, (v_mod->>'quantidade')::int, 0, 0,
                coalesce(v_preco, 0), true,
                CASE WHEN v_oem_prod IS NOT NULL THEN 'oem' ELSE 'intake' END);
      END LOOP;

      IF array_length(v_sem_preco, 1) > 0 THEN
        v_avisos := v_avisos || jsonb_build_object('campo','produtos[].modulos','aviso',
          'custo_sem_tabela: ' || array_to_string(v_sem_preco, ', ') ||
          ' não têm preço na tabela do OEM e entraram com custo zero');
      END IF;
      ...
      -- Olha o que ficou em branco NA FICHA, nao o que faltou no payload.
      -- Desde 04/09 o fornecedor pode vir do cadastro do produto: cobrar do
      -- payload acusaria falta de algo que esta preenchido, e um aviso que
      -- grita sem motivo treina todo mundo a ignorar o aviso verdadeiro.
      -- vlr_custo saiu da lista: o Custo Operacao e informado depois, por
      -- outro caminho, e nao e responsabilidade de quem manda a proposta.
      v_faltando := '{}';
      SELECT cp.fornecedor_id, cp.modelo_contrato_id, cp.recorrencia::text
        INTO v_f_fornec, v_f_modelo, v_f_recor
        FROM cliente_produtos cp WHERE cp.id = v_cp;
      IF v_f_fornec IS NULL THEN v_faltando := array_append(v_faltando, 'fornecedor_id'); END IF;
      IF v_f_modelo IS NULL THEN v_faltando := array_append(v_faltando, 'modelo_contrato_id'); END IF;
      IF v_f_recor  IS NULL THEN v_faltando := array_append(v_faltando, 'recorrencia'); END IF;
      IF array_length(v_faltando,1) > 0 THEN
        v_avisos := v_avisos || jsonb_build_object('campo','produtos[]','produto_id',v_prod->>'produto_id',
          'aviso','contrato_incompleto: o produto entrou sem ' || array_to_string(v_faltando, ', ') ||
                  '. Esses campos ficaram em branco na ficha do produto');
      END IF;
    END LOOP;
  ...
  IF v_modo = 'venda_nova' THEN
    INSERT INTO onboarding_journey_modules (tenant_id, journey_id, nome, produto_modulo_id, quantidade, origem, position)
    SELECT v_tenant, v_journey, pm.nome, pm.id, greatest(coalesce(m.quantidade, 1), 1), 'intake',
