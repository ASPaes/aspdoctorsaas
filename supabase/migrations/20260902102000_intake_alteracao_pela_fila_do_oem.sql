...
-- ============================================================================
-- A calculadora passa a usar o caminho canônico do DoctorSaaS.
--
-- Até aqui o modo `alteracao` da `fn_intake_proposta` escrevia direto na ficha:
--
--   INSERT cliente_produto_modulos (..., vlr_mensal=0, vlr_ativacao=0, 'intake')
--   INSERT movimentos_mrr (descricao do payload, current_date)
--
-- Nada disso passava pela `oem_sync_fila`. Medido em produção (31/08–01/09/2026,
-- tenant Digi Office):
--
--   · SORVETE REAL 2 (filial 23272) — módulo Estoque entrou na ficha com ZERO
--     linhas na fila. O parceiro nunca soube.
--   · CASA BACURAU (filial 22255) — +15 Licença PDV na ficha, nada no OEM, e o
--     espelho reverteu -15 no dia seguinte. Sobrou R$ 540 de up-sell no MRR sem
--     módulo nenhum atrás.
--   · COLHER DE PAU (filial 14546) — cancelou Delivery legal e 3 PDVs; o espelho
--     desfez às 00:17 do dia seguinte.
--
-- A partir daqui, produto COM licença no parceiro nunca é gravado aqui: o
-- pedido vira linha na fila (`aguardando_aprovacao`) e quem cria o módulo, sobe
-- a quantidade, dá a baixa e lança o MRR é a `fn_oem_fila_aplicar`, depois da
-- aprovação e do aceite do OEM. É a mesma ordem — OEM primeiro, ficha depois —
-- que a tela do cliente segue desde 21/08/2026.
--
-- ⚠️ CONSEQUÊNCIA ACEITA PELO OWNER (02/09/2026): o MRR da calculadora deixa de
-- entrar na chegada do webhook e passa a entrar depois da aprovação. Isso
-- reverte a decisão de 27/08 ("MRR entra na chegada"). Recusa do admin agora
-- significa venda sem MRR — que é o mesmo que já vale para a tela.
--
-- ---------------------------------------------------------------------------
-- Três outras coisas mudam junto, porque sem elas o caminho novo entrega pior:
--
-- 1. VALOR POR MÓDULO. A fila só lança movimento quando o payload diz
--    `vlr_mensal`; sem isso o módulo iria ao parceiro e o MRR sumiria. O bloco
--    `alteracao` passa a ACEITAR `vlr_mensal` e `vlr_ativacao` por módulo
--    (unitário) — a calculadora já sabe esses números, ela os manda em
--    `proposta.itens[].mrr/setup` e o contrato é que não deixava usá-los.
--    Com UM módulo só, o valor da alteração é o dele e é derivado aqui. Com
--    dois ou mais, o payload precisa dizer: dividir um total por quantidade
--    seria inventar preço.
--
-- 2. GUARDA DE CONTRADIÇÃO. Em 31/08 a COLHER DE PAU chegou com
--    `proposta.ticket_type = "upsell"`, `valor_mrr = 389` e itens "Ponto
--    adicional x3" + "Delivery legal x1" — uma COMPRA — mas o bloco `alteracao`
--    veio invertido: `valor_delta: -389`, deltas -3 e -1. O DoctorSaaS obedeceu
--    e cancelou os módulos de um cliente que estava comprando. Barrar a
--    contradição custa uma recusa registrada; deixá-la passar custa a licença.
--
-- 3. DATA DA VENDA. O movimento nasce com `alteracao.data_venda` (ou hoje), e
--    esse carimbo viaja no payload da fila. Sem ele, um pedido aprovado três
--    dias depois cairia no mês da aprovação, não no da venda.
-- ============================================================================
