-- =============================================================================
-- CORREÇÃO DA CORREÇÃO: dentro do ON CONFLICT vale o ALIAS, não o nome.
--
-- A migration anterior (40705e86) ensinou a detecção a preservar a escolha
-- humana, e QUEBROU a reconstrução inteira ao fazê-lo: o insert é
-- `INSERT INTO reconciliacao_cadastro AS r`, e eu referenciei a linha antiga
-- pelo nome da tabela. O Postgres respondeu:
--
--   invalid reference to FROM-clause entry for table "reconciliacao_cadastro"
--
-- ⚠️ E O ERRO NÃO APARECEU EM LUGAR NENHUM ÓBVIO. O cron continuou marcando
-- "succeeded" (ele só enfileira a requisição), a edge function continuou
-- devolvendo `ok: true, resultado: espelho_atualizado`, e a falha viajava
-- escondida dentro do corpo da resposta, num campo `deteccao_erro`. A
-- reconciliação ficou 25 minutos sem reconstruir e nada gritou.
--
-- Só apareceu porque fui conferir se os 211 tinham sobrevivido, em vez de
-- confiar que sim. A lição é essa: "o cron rodou" e "o cron funcionou" são
-- afirmações diferentes, e aqui elas se separam por três camadas.
--
-- Conferido depois de aplicar: reconstrução em 26/09 02:22:29, os 211
-- sobreviveram, e os contratos vinculados subiram de 791 para 1.016.
-- =============================================================================
...
BEGIN
  ...
  ON CONFLICT (tenant_id, ds_contract_id) DO UPDATE SET
    ...
    codigo_cliente_omie=EXCLUDED.codigo_cliente_omie, codigo_contrato_omie = case
      ...
      when r.status_usuario in ('resolvido','vinculado')
       and r.candidato_escolhido is not null
        then coalesce(EXCLUDED.codigo_contrato_omie, r.candidato_escolhido)
