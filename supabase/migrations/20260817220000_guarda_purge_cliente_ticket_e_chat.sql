-- 17/08/2026 — guarda do purge em admin_delete_cliente.
--
-- Caso que originou: em 13/08 um cadastro DUPLICADO de "ESQUINA DO CHOPP JARA LTDA"
-- foi excluido no modo 'purge' com "incluir chat" ligado. Levou junto o TK-2026-3061
-- (aberto no dia anterior) e a conversa inteira do WhatsApp do contato do cliente.
-- A operadora foi falar com o cliente e nao achou nem o ticket nem o chat.
--
-- Por que o estrago e total e silencioso:
--   delete support_tickets   -> support_ticket_events ON DELETE CASCADE  (timeline some)
--   delete whatsapp_contacts -> whatsapp_conversations ON DELETE CASCADE
--                                 -> whatsapp_messages, whatsapp_reactions,
--                                    whatsapp_conversation_notes/_summaries,
--                                    whatsapp_sentiment_*, conversation_assignments,
--                                    support_attendances — todos ON DELETE CASCADE
-- Sobra 1 linha em audit_events com as contagens. support_tickets.deleted_at existe
-- e nao era usado. Recuperacao so por PITR.
--
-- Para cadastro duplicado o modo correto ja existia: 'transfer' reaponta contratos,
-- tickets, atendimentos e contatos para o sobrevivente e so entao apaga o cliente.
-- Nada obrigava a usar. Esta migration passa a obrigar.
--
-- p_forcar existe para o caso legitimo (lixo de importacao, teste) e a tela NAO o
-- expoe: quem precisa forcar passa pelo SQL Editor, de proposito.
--
-- DROP + CREATE porque a assinatura ganhou p_forcar. Os GRANTs sao repostos abaixo:
-- DROP nao os carrega de volta, e sem eles o RPC volta null no frontend.

DROP FUNCTION IF EXISTS public.admin_delete_cliente(uuid, text, uuid, boolean, boolean);

CREATE OR REPLACE FUNCTION public.admin_delete_cliente(
  "p_cliente_id" uuid,
  "p_mode" text,
  "p_target_id" uuid DEFAULT NULL::uuid,
  "p_confirm" boolean DEFAULT false,
  "p_incluir_chat" boolean DEFAULT true,
...
REVOKE ALL ON FUNCTION public.admin_delete_cliente(uuid, text, uuid, boolean, boolean, boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION public.admin_delete_cliente(uuid, text, uuid, boolean, boolean, boolean) TO "authenticated";
GRANT ALL ON FUNCTION public.admin_delete_cliente(uuid, text, uuid, boolean, boolean, boolean) TO "service_role";
