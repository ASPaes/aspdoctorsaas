...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: settings, error: settingsErr } = await supabase
      .from("ai_settings")
      .select("id, provider, model, base_url, api_key_encrypted, system_prompt")
    ...
    const { data: apiKey, error: decryptErr } = await supabase.rpc(
      "decrypt_api_key",
      { p_encrypted: settings.api_key_encrypted, p_encryption_key: encryptionKey }
    );
    ...
    console.log(`[${FUNCTION_NAME}][${requestId}] Resultado: ok=${testOk}, latency=${latencyMs}ms`);

    await supabase
      .from("ai_settings")
      .update({ last_tested_at: new Date().toISOString(), last_test_ok: testOk, last_test_error: testError })
      .eq("id", settings.id);
    ...
    await supabase.from("audit_events").insert({
      ...
      metadata: { provider, model: modelUsed, test_ok: testOk, latency_ms: latencyMs, error_summary: testError ? testError.slice(0, 100) : null },
    });
