...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (note.media_path) {
      ...
      await service.storage.from('whatsapp-media').remove([note.media_path]);
    }

    const { error: delErr } = await userClient.from('whatsapp_conversation_notes').delete().eq('id', note_id);
    if (delErr) return new Response(JSON.stringify({ error: 'Failed to delete note' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
