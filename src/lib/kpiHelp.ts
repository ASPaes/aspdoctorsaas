...
const kpiHelp: Record<string, KpiHelpEntry> = {
  ...
  atendimento_ag_total: {
    ...
    definition: "Total de atendimentos atribuídos ao agente que foram abertos no período — incluindo os que ainda estão em andamento.",
    why_it_matters: "Volume de trabalho que chegou pro agente. Cruzado com encerrados, mostra quanto da carga ele já fechou.",
    formula: "COUNT dos atendimentos do agente com abertura no período (apenas conversas com mensagem do cliente)",
  },
  atendimento_ag_encerrados: {
    title: "Encerrados (Encerr.)",
  ...
  atendimento_ag_pico: {
    ...
    definition: "Maior número de atendimentos que o agente teve abertos ao mesmo tempo no período.",
    why_it_matters: "Mede sobrecarga de pico. Muitos chats simultâneos derrubam a qualidade e a velocidade de resposta.",
    formula: "máximo de atendimentos com janelas (assumido…encerrado) sobrepostas do agente",
    example: "Pico 4 = em algum momento o agente segurou 4 conversas ao mesmo tempo",
  },
  atendimento_ag_tma: {
  ...
  atendimento_ag_faixa: {
    ...
    definition: "A faixa de tempo de resposta em que o agente mais cai (a moda) — o comportamento mais frequente, não a média.",
    why_it_matters: "Complementa a latência mediana: mostra onde o agente normalmente está — respondendo em <30s na maioria das vezes, ou tipicamente em 5–10min?",
    formula: "faixa de latência mais frequente do agente (moda), entre <30s, 30s–1min, 1–2min, 2–5min, 5–10min, 10–30min e 30min+",
  },
  atendimento_ag_csat: {
  ...
  atendimento_ag_reabertura: {
    title: "Taxa de Reabertura",
    definition: "% dos atendimentos encerrados pelo agente que voltaram a ser abertos (o problema retornou).",
    why_it_matters: "É o inverso da resolução no 1º contato (FCR). Reabertura alta indica problema mal resolvido na primeira vez. Acima de 10% acende alerta.",
    formula: "atendimentos encerrados reabertos ÷ atendimentos encerrados do agente",
  },
  atendimento_ag_msgs: {
    title: "Mensagens por Atendimento",
    definition: "Média de mensagens que o agente envia por atendimento encerrado.",
    why_it_matters: "Eficiência da comunicação. Muito alto pode ser enrolação; muito baixo, frieza ou resposta incompleta. Leia junto com CSAT e TMA.",
    formula: "média de mensagens enviadas pelo agente nos atendimentos encerrados (com pelo menos 1 mensagem)",
  },
  ...
  atendimento_backlog_abertos: { title: "Backlog aberto", definition: "Tickets em status não-terminal e não deletados, contados agora (independe do período do filtro).", why_it_matters: "É o tamanho real da fila pendente. Crescimento sustentado indica capacidade insuficiente ou gargalo de triagem.", formula: "COUNT(tickets WHERE status.is_terminal=false AND deleted_at IS NULL)" },
  atendimento_backlog_orfaos: { title: "Tickets órfãos", definition: "Tickets abertos sem responsável atribuído.", why_it_matters: "Ticket sem dono tende a estagnar — ninguém está olhando. Deve ficar perto de zero.", formula: "COUNT(abertos WHERE responsavel_user_id IS NULL)" },
  atendimento_backlog_parados: { title: "Tickets parados", definition: "Tickets abertos sem nenhuma atualização há mais de 7 dias.", why_it_matters: "Mede negligência/esquecimento na fila. Alto = SLA em risco e cliente sem resposta.", formula: "COUNT(abertos WHERE atualizado_em < agora - 7 dias)" },
  atendimento_backlog_vencidos: { title: "Tickets vencidos", definition: "Tickets abertos que já passaram da previsão de encerramento (previsao_encerramento).", why_it_matters: "Quebra de SLA explícita. Atenção: depende de a previsão estar setada de forma realista — se o tenant usa previsão muito curta (ex.: poucas horas), quase todo ticket aberto aparecerá como vencido.", formula: "COUNT(abertos WHERE agora > previsao_encerramento)" },
  atendimento_backlog_aging: { title: "Aging do backlog", definition: "Distribuição dos tickets abertos por faixa de idade desde a abertura: 0–2d, 3–7d, 8–30d, +30d.", why_it_matters: "É o indicador mais honesto de saúde do backlog. Concentração nas faixas longas (8–30d, +30d) revela acúmulo crônico, mesmo quando o volume total parece baixo.", formula: "idade = agora - aberto_em, agrupado em faixas" },
  atendimento_backlog_prioridade: { title: "Backlog por prioridade", definition: "Tickets abertos por nível de prioridade (urgente, alta, média, baixa), com a contagem dos que já venceram.", why_it_matters: "Vencido em urgente/alta é muito mais grave que em baixa. Ajuda a priorizar o que atacar primeiro.", formula: "COUNT(abertos) e COUNT(vencidos) GROUP BY prioridade" },
  atendimento_backlog_status: { title: "Backlog por status", definition: "Tickets abertos distribuídos pelos status não-terminais configurados pelo tenant.", why_it_matters: "Mostra onde a fila trava: muitos em 'aguardando cliente' aponta dependência externa; muitos em 'novo' aponta gargalo de triagem.", formula: "COUNT(abertos) GROUP BY status" },
  atendimento_backlog_plantao: { title: "Plantão por produto", definition: "Tickets abertos no período em horário de plantão (fora do comercial), agrupados por produto. O cabeçalho mostra o total plantão vs comercial.", why_it_matters: "Identifica quais produtos geram demanda fora do horário — peso de plantão concentrado em um produto é sinal de instabilidade ou de cobertura mal dimensionada.", formula: "COUNT(tickets no período WHERE tipo_horario='plantao') GROUP BY produto" },
  atendimento_cob_chats: { title: "Chats (todos os tenants)", definition: "Total de atendimentos de chat abertos no período somando toda a base de tenants.", why_it_matters: "Dá a escala da operação de atendimento da plataforma inteira — base para os percentuais de cobertura.", formula: "COUNT(support_attendances no período), todos os tenants" },
  atendimento_cob_cliente: { title: "% com cliente vinculado", definition: "Percentual de chats que têm um cliente (cliente_id) associado, somando todos os tenants.", why_it_matters: "É o principal bloqueador dos cruzamentos chat×cliente×MRR (custo de suporte por conta, health score). Chat sem cliente não entra nessas análises. Tenant com % baixo precisa do vínculo em lote.", formula: "chats com cliente_id ÷ total de chats" },
  atendimento_cob_ticket: { title: "% que vira ticket", definition: "Percentual de chats vinculados a um ticket (ticket_id), medindo a adoção do fluxo integrado chat→ticket.", why_it_matters: "Mostra quais tenants conectam o atendimento de chat ao backlog estruturado de tickets. Hoje é concentrado em poucos tenants.", formula: "chats com ticket_id ÷ total de chats" },
  atendimento_cob_tenants: { title: "Tenants ativos", definition: "Quantidade de tenants com pelo menos um chat ou ticket no período.", why_it_matters: "Mede a base efetivamente em operação no atendimento dentro da janela analisada.", formula: "COUNT(DISTINCT tenant com chats>0 OU tickets>0)" },
  atendimento_cob_maturidade: { title: "Maturidade de dados por tenant", definition: "Heurística que conta quantas de 5 capacidades o tenant tem ativas pelos dados: cliente vinculado ≥50%, usa tickets, bridge chat→ticket ≥10%, coleta CSAT ≥15%, usa URA ≥30%. 0–1 = Inicial, 2–3 = Intermediário, 4–5 = Avançado.", why_it_matters: "Régua rápida da jornada de adoção — onde cada tenant está e o que falta ligar pra desbloquear análises mais ricas (ex.: vincular cliente desbloqueia o cruzamento com MRR). É uma heurística: as colunas % ao lado são o sinal real e os limiares são ajustáveis.", formula: "soma de capacidades ativas (0–5) por limiar" },
  atendimento_cli_clientes: { title: "Clientes atendidos", definition: "Quantidade de clientes distintos com pelo menos um chat ou ticket no período (apenas atendimentos com cliente vinculado).", why_it_matters: "Base da visão por conta. Cresce conforme a cobertura de vínculo de cliente melhora.", formula: "COUNT(DISTINCT cliente_id) com chat ou ticket no período" },
  atendimento_cli_mrr: { title: "MRR coberto", definition: "Soma do MRR ativo (cliente_produtos.vlr_mensal onde ativo=true) dos clientes que apareceram no atendimento no período.", why_it_matters: "Mostra quanto de receita recorrente está representado na tabela — contexto para o custo de suporte.", formula: "SUM(vlr_mensal ativo) dos clientes atendidos" },
  atendimento_cli_densidade: { title: "Densidade de suporte (int/R$1k MRR)", definition: "Interações (chats + tickets) a cada R$ 1.000 de MRR do cliente. Ranqueia ofensores por PESO RELATIVO, não volume bruto — cliente grande abre mais e está certo. Clientes com MRR abaixo de R$ 50 (placeholders tipo R$ 1) ficam fora do ranking e da média (mostrados como '—'), pois não refletem receita real.", why_it_matters: "Aponta quem consome suporte desproporcional ao que paga — candidato a revisão de plano, treinamento ou churn. É o indicador central da aba.", formula: "interações × 1000 ÷ MRR (somente MRR ≥ R$ 50)" },
  atendimento_cli_ofensor: { title: "Ofensor #1", definition: "Cliente com maior densidade de suporte no período (interações por R$1k de MRR, respeitando o piso de R$ 50).", why_it_matters: "Atalho para a conta que mais pesa proporcionalmente — onde olhar primeiro.", formula: "cliente com MAX(interações ÷ MRR)" },
  atendimento_cli_risco: { title: "Risco de churn (por conta)", definition: "Contagem de até 4 sinais de risco por cliente: (1) densidade de suporte acima de N× a média do tenant, (2) % de chats com sentimento negativo acima do limiar, (3) reincidência de categorias de ticket, (4) CSAT médio abaixo do limiar (só quando há nota suficiente). 0 = Baixo, 1–2 = Médio, 3–4 = Alto.", why_it_matters: "Aponta contas que combinam custo de suporte desproporcional, insatisfação e problemas recorrentes — perfil típico de pré-churn. É uma heurística (não um modelo validado contra cancelamento) e os limiares são configuráveis por tenant em configuracoes.support_config.", formula: "soma de flags disparadas (0–4); limiares por tenant com fallback global" },
};
