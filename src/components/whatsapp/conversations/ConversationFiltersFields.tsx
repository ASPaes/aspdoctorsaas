...
import { Sparkles, Check, ChevronsUpDown, HelpCircle, ShieldOff, Info } from "lucide-react";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { cn } from "@/lib/utils";
import { useWhatsAppInstances } from "../hooks/useWhatsAppInstances";
import { useAuth } from "@/contexts/AuthContext";
import { useAgentOptions } from "../hooks/useAgentOptions";
import type { FiltersState, SortBy } from "./ConversationFiltersPopover";
...
export const FILTROS_VAZIOS: FiltersState = {
  sortBy: "recent",
  status: undefined,
  instanceId: undefined,
  assignedToMe: false,
  assignedToAgent: undefined,
  autoReplyDisabledOnly: false,
  rulesDisabledOnly: false,
  groupByAgent: false,
};

export function contarFiltrosAtivos(filters: FiltersState): number {
  return (
...
interface Props {
  ...
  /** Fecha o container (popover no desktop, folha no celular) ao sair pelo link da URA. */
  onNavigateAway?: () => void;
}

/**
 * Os campos dos filtros, sem o container. O desktop os mostra num popover e o
 * celular numa folha de baixo — mas são os MESMOS controles, para não existir
 * filtro que só uma das telas tem.
 */
export function ConversationFiltersFields({
  filters,
  onChange,
  showGroupByAgent = false,
  operatorFilterInactive = false,
  onNavigateAway,
}: Props) {
