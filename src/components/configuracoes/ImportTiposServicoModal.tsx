...
export default function ImportTiposServicoModal({ open, onOpenChange, tenantId, onSuccess }: Props) {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      // Dedupe by nome (case-insensitive) within tenant
      const { data: existing, error: fetchErr } = await (supabase.from("service_types" as any) as any)
        .select("nome")
        .eq("tenant_id", tenantId);
      if (fetchErr) throw fetchErr;
      const existingNames = new Set((existing ?? []).map((r: any) => String(r.nome).toLowerCase()));

      const toInsert = validRows.filter(r => !existingNames.has(r.nome.toLowerCase()));
      ...
      for (let i = 0; i < payload.length; i += batchSize) {
        ...
        const { error } = await (supabase.from("service_types" as any) as any).insert(batch);
        if (error) throw error;
      }
      toast({ title: `${toInsert.length} tipos importados! (${skipped} já existiam)` });
      onSuccess();
      reset();
      onOpenChange(false);
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Importar Tipos de Serviço via CSV</DialogTitle>
          <DialogDescription>
            Formato: arquivo CSV com separador ponto-e-vírgula (;). Colunas: codigo;nome;descricao
          </DialogDescription>
        </DialogHeader>
          ...
          {!fileName ? (
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <Checkbox id="hasHeaderTipos" checked={hasHeader} onCheckedChange={(c) => setHasHeader(!!c)} />
                <label htmlFor="hasHeaderTipos" className="text-sm">Primeira linha é cabeçalho</label>
              </div>
              ...
              <p className="text-sm text-muted-foreground">
                {validRows.length} tipos válidos de {dataRows.length} linhas
              </p>
            </div>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleImport} disabled={validRows.length === 0 || importing}>
            ...
            Importar {validRows.length} tipos
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
