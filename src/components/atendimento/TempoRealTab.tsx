...
import { Loader2, Users, AlertTriangle, Clock, Timer, MessageSquare, Hourglass } from "lucide-react";
...
export function TempoRealTab() {
  ...
  const verChats = (bucket: string, title: string) => (
    <button
      type="button"
      onClick={() => setVerBucket({ bucket, title })}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        <KPICardEnhanced
          ...
          label="Fila Agora"
          value={data.fila}
          subtitle={data.fila_fora_hora > 0 ? `+${data.fila_fora_hora} fora do horário` : "dentro do horário"}
          icon={<Hourglass className="h-4 w-4" />}
          footer={data.fila > 0 ? verChats("fila", "Fila Agora") : undefined}
        />
        <KPICardEnhanced
        ...
        <KPICardEnhanced
          ...
          variant={data.sla_estourando > 0 ? "destructive" : "dark"}
          subtitle="acima do limite de 1ª resposta"
          icon={<Timer className="h-4 w-4" />}
          footer={data.sla_estourando > 0 ? verChats("sla_estourando", "Estourando SLA") : undefined}
        />
        <KPICardEnhanced
          helpKey="atendimento_parados_24h"
          label="Parados > 24h"
          value={data.parados_24h}
          variant={data.parados_24h > 0 ? "warning" : "dark"}
          subtitle="precisam de ação imediata"
          icon={<AlertTriangle className="h-4 w-4" />}
          footer={data.parados_24h > 0 ? verChats("parados_24h", "Parados > 24h") : undefined}
        />
      </div>

      {/* Listas (painéis limpos, padrão do financeiro) */}
          ...
          {data.ativos_por_depto.length === 0 ? (
            ...
            <ul className="divide-y divide-border">
              {data.ativos_por_depto.map((d) => (
                <li key={d.department_id ?? "sem-depto"} className="flex items-center justify-between py-2">
