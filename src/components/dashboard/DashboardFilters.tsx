    ...
    <div className={`flex flex-wrap items-end gap-3 ${tvMode ? 'p-4' : ''}`}>
      ...
      <div className="space-y-1">
        ...
        <Select value={autoRefreshInterval.toString()} onValueChange={v => onAutoRefreshChange(Number(v))}>
          <SelectTrigger className={cn('w-[110px]', tvMode && 'h-12 text-lg')}><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      {loading && <Badge variant="secondary" className="animate-pulse mb-0.5">Carregando...</Badge>}
    </div>
  );
}
