...
export interface UseCohortRevenueResult {
  isLoading: boolean;
  cohorts: CohortEntry[];
  ageColumns: number[];
  matrix: Map<string, Map<number, number>>;
  revenueMatrix: Map<string, Map<number, number>>;
...
export function useCohortRevenue(params: CohortRevenueParams = {}): UseCohortRevenueResult {
  ...
  return useMemo(() => {
    const empty: UseCohortRevenueResult = {
      isLoading,
      cohorts: [],
      ageColumns: [],
      matrix: new Map(),
      revenueMatrix: new Map(),
    ...
    const filtered = rawData.filter(r => r.grupo === 'Geral');
    if (filtered.length === 0) return empty;

    const cohortsMap = new Map<string, number>();
    const agesSet = new Set<number>();
    const matrix = new Map<string, Map<number, number>>();
    const revenueMatrix = new Map<string, Map<number, number>>();
    ...
    for (const row of filtered) {
      ...
      const revPct = Number(row.revenue_retention_percent);
      ...
      if (!revenueMatrix.has(cm)) revenueMatrix.set(cm, new Map());
      revenueMatrix.get(cm)!.set(age, revPct);
    ...
    return { isLoading, cohorts, ageColumns, matrix, revenueMatrix, retainedMatrix, curveData, curveLabels, curveIsFallback };
  }, [rawData, isLoading]);
}
