...
import { TrendingUp, TrendingDown, UserMinus, UserPlus, Equal, ShoppingCart, RefreshCw, Percent } from 'lucide-react';
...
export function NetNewMrrBreakdown({ newMrr, upsellMrr, crossSellMrr, reativacaoMrr, reajusteMrr, downsellMrr, mrrCancelado, netNewMrr, tvMode = false }: NetNewMrrBreakdownProps) {
  const items = [
    ...
    { label: 'Reativação', value: reativacaoMrr, icon: RefreshCw, color: reativacaoMrr === 0 ? 'text-muted-foreground' : 'text-green-700 dark:text-green-400', bgColor: reativacaoMrr === 0 ? 'bg-muted/50' : 'bg-green-500/10', sign: '+', tooltip: 'MRR recuperado por reativação de clientes cancelados' },
    { label: 'Reajuste', value: reajusteMrr, icon: Percent, color: reajusteMrr === 0 ? 'text-muted-foreground' : 'text-blue-600 dark:text-blue-400', bgColor: reajusteMrr === 0 ? 'bg-muted/50' : 'bg-blue-500/10', sign: '+', tooltip: 'MRR adicionado por reajustes de contratos' },
      ...
      <CardContent>
        ...
        <TooltipProvider>
          ...
          <div className="flex items-center justify-center gap-1.5 py-3 border-t border-border flex-wrap font-mono">
            ...
            <span className="text-muted-foreground font-bold">+</span>
            <span className={cn(reativacaoMrr === 0 ? 'text-muted-foreground' : 'text-green-700 dark:text-green-400', 'font-medium', tvMode ? 'text-base' : 'text-sm')}>{fmt(reativacaoMrr)}</span>
            <span className="text-muted-foreground font-bold">+</span>
            <span className={cn(reajusteMrr === 0 ? 'text-muted-foreground' : 'text-blue-600 dark:text-blue-400', 'font-medium', tvMode ? 'text-base' : 'text-sm')}>{fmt(reajusteMrr)}</span>
