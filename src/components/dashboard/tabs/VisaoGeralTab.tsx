    ...
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className={`grid gap-4 ${tvMode ? 'grid-cols-2 lg:grid-cols-6' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-6'}`}>
          <KPICardEnhanced label="Vendas no Período" value={certA1?.vendasQtd?.toLocaleString('pt-BR') || '0'} size={tvMode ? 'lg' : 'md'} variant="primary" icon={<ShieldCheck className={`${tvMode ? 'h-6 w-6' : 'h-4 w-4'} text-current`} />} helpKey="cert_vendas_periodo" />
          <KPICardEnhanced label="Perdido p/ Terceiro" value={certA1?.perdidoTerceiroQtd?.toLocaleString('pt-BR') || '0'} size={tvMode ? 'lg' : 'md'} variant={certA1?.perdidoTerceiroQtd && certA1.perdidoTerceiroQtd > 0 ? 'destructive' : 'default'} icon={<UserX className={`${tvMode ? 'h-6 w-6' : 'h-4 w-4'} text-current`} />} helpKey="cert_perdido_terceiro" />
          <KPICardEnhanced label="Faturamento A1" value={fmt(certA1?.faturamento || 0)} size={tvMode ? 'lg' : 'md'} variant="primary" icon={<DollarSign className={`${tvMode ? 'h-6 w-6' : 'h-4 w-4'} text-current`} />} helpKey="cert_faturamento_a1" />
          <KPICardEnhanced label="Oportunidades (Janela)" value={certA1?.oportunidadesJanela?.toLocaleString('pt-BR') || '0'} size={tvMode ? 'lg' : 'md'} variant="default" icon={<Target className={`${tvMode ? 'h-6 w-6' : 'h-4 w-4'} text-current`} />} subtitle="Baseado em hoje" helpKey="cert_oportunidades_janela" />
          <KPICardEnhanced label="Vencendo em 30 dias" value={certA1?.oportunidadesVencendo?.toLocaleString('pt-BR') || '0'} size={tvMode ? 'lg' : 'md'} variant="warning" icon={<Clock className={`${tvMode ? 'h-6 w-6' : 'h-4 w-4'} text-current`} />} subtitle="Baseado em hoje" helpKey="cert_vencendo_30d" className="ring-2 ring-warning/40" />
          <KPICardEnhanced label="Vencidos até 20 dias" value={certA1?.oportunidadesVencidas?.toLocaleString('pt-BR') || '0'} size={tvMode ? 'lg' : 'md'} variant={certA1?.oportunidadesVencidas && certA1.oportunidadesVencidas > 0 ? 'destructive' : 'default'} icon={<AlertTriangle className={`${tvMode ? 'h-6 w-6' : 'h-4 w-4'} text-current`} />} subtitle="Baseado em hoje" helpKey="cert_vencidos_20d" className={certA1?.oportunidadesVencidas && certA1.oportunidadesVencidas > 0 ? 'ring-2 ring-red-500/40' : ''} />
        </div>
      </section>
