  ...
  return (
    <div className="space-y-6">
      <div className={`grid gap-4 ${tvMode ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'}`}>
        ...
        <KPICardEnhanced label="Net New MRR (no período)" value={fmt(metrics.netNewMrr)} icon={<DollarSign className={`${tvMode ? 'h-8 w-8' : 'h-5 w-5'} text-primary`} />} size={s} variant={metrics.netNewMrr >= 0 ? 'success' : 'destructive'} subtitle="Variação líquida no período" formula="New MRR (novos clientes por data_venda) + Upsell + Cross-sell − Downsell − Churn MRR. Diferente do MRR Snapshot, que é a foto atual." />
        <KPICardEnhanced label="MC Total (R$)" value={mc != null ? fmt(mc.mc_total) : '—'} icon={<Calculator className={`${tvMode ? 'h-8 w-8' : 'h-5 w-5'} text-primary`} />} size={s} variant="dark" formula="Receita (MRR) − COGS − Impostos − Custos Fixos alocados. Margem de contribuição total da carteira ativa." />
      </div>
