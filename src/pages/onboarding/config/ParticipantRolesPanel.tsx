...
import { Plus, GripVertical, Trash2, Loader2, Lock, Building2 } from "lucide-react";
import {
...
/** Radix Select não aceita item de valor vazio: este é o "sem setor". */
const SEM_SETOR = "__nenhum__";

function SortableRow({
  item, setores, setorTomadoPor, onToggle, onRename, onRecolor, onDelete, onSetSetor,
}: {
  item: OnboardingParticipantRole;
  setores: Setor[];
  /** setor -> nome do papel que já o ocupa. Um setor aponta para um papel só. */
  setorTomadoPor: Map<string, string>;
  onToggle: (id: string, v: boolean) => void;
  onRename: (id: string, nome: string) => void;
  onRecolor: (id: string, cor: string) => void;
  onDelete: (id: string) => void;
  onSetSetor: (id: string, departmentId: string | null) => void;
}) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="p-2 rounded-md border border-border bg-card space-y-1.5">
      ...
      <div className="flex items-center gap-2 pl-8">
        ...
        <span className="text-[11px] text-muted-foreground shrink-0">Setor</span>
        <Select
          value={item.department_id ?? SEM_SETOR}
          onValueChange={(v) => onSetSetor(item.id, v === SEM_SETOR ? null : v)}
        >
          <SelectTrigger className="h-7 text-xs">
            <SelectValue placeholder="Nenhum" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={SEM_SETOR} className="text-xs">Nenhum</SelectItem>
            {setores.map((s) => {
              const tomadoPor = setorTomadoPor.get(s.id);
              const bloqueado = !!tomadoPor && s.id !== item.department_id;
              return (
                <SelectItem key={s.id} value={s.id} disabled={bloqueado} className="text-xs">
                  {s.name}{bloqueado ? ` · já é de "${tomadoPor}"` : ""}
                </SelectItem>
              );
            })}
          </SelectContent>
...
export function ParticipantRolesPanel() {
  ...
  const setoresQ = useQuery({
    queryKey: ["onb-papeis-setores", effectiveTenantId],
  ...
  const setorTomadoPor = new Map(
    items.filter((i) => i.department_id).map((i) => [i.department_id as string, i.nome]),
  );
  ...
  async function patch(id: string, campos: Record<string, unknown>) {
    ...
    if (error) {
      toast.error(
        error.code === "23505" && "department_id" in campos
          ? "Esse setor já está ligado a outro papel."
          : error.code === "23505"
